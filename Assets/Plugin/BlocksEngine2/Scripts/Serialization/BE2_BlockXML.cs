...
namespace MG_BlocksEngine2.Serializer
{
    public static class BE2_BlockXML
    {
	    ...
	    // v2.12 - added xml elements to the BE2_BlockXML to enable the serialization of Function Blocks
            xBlock.Add(new XElement("defineID", serializableBlock.defineID));
            xBlock.Add(new XElement("isLocalVar", serializableBlock.isLocalVar));

            XElement xDefineItems = new XElement("defineItems");
            xBlock.Add(xDefineItems);
            foreach (DefineItem item in serializableBlock.defineItems)
            {
                XElement xItem = new XElement("Item");
                xDefineItems.Add(xItem);
                xItem.Add(new XElement("type", item.type));
                xItem.Add(new XElement("value", item.value));
            }
        ...
        public static BE2_SerializableBlock XElementToSBlock(XElement xBlock)
        {
            ...
            // v2.12 - added xml elements to the BE2_BlockXML to enable the serialization of Function Blocks
            sBlock.defineID = xBlock.Element("defineID").Value;
            sBlock.isLocalVar = xBlock.Element("isLocalVar").Value;

            IEnumerable<XElement> xDefineItems = xBlock.Element("defineItems").Elements("Item");
            foreach (XElement xItem in xDefineItems)
            {
                sBlock.defineItems.Add(new DefineItem(xItem.Element("type").Value, xItem.Element("value").Value));
            }
